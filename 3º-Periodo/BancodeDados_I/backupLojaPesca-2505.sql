...
-- Copiando dados para a tabela lojapesca.auditoria: ~8 rows (aproximadamente)
...
BEGIN
	SELECT COUNT(*) INTO @contador FROM produto WHERE codProduto = codProdutoR;
...
-- Copiando estrutura para procedure lojapesca.proc_relatorioVendasCliente
DROP PROCEDURE IF EXISTS `proc_relatorioVendasCliente`;
DELIMITER //
CREATE PROCEDURE `proc_relatorioVendasCliente`(
	IN `codClientePesquisar` INT
)
BEGIN
	SELECT COUNT(*) INTO @contador FROM venda WHERE CLIENTE_codCliente = codClientePesquisar;
	
	if(@contador = 0)
		then SELECT "Cliente não realizou nenhuma compra ainda!" AS Erro;
	else
		SELECT v.codVenda, v.tipo, DATE_FORMAT(v.dataHoraVenda, "%d/%m/%Y %H:%i:%s") AS dataHora, c.nomeCliente,
		c.cpf FROM venda AS v INNER JOIN cliente AS c 
		ON v.CLIENTE_codCliente = c.codCliente WHERE v.CLIENTE_codCliente = codClientePesquisar ORDER BY v.codVenda;
	END if;
END//
DELIMITER ;

-- Copiando estrutura para procedure lojapesca.proc_updateProduto
DROP PROCEDURE IF EXISTS `proc_updateProduto`;
DELIMITER //
CREATE PROCEDURE `proc_updateProduto`(
	IN `codProdutos` int,
	IN `nomeProduto` varchar(50),
	IN `qntdEstoque` int,
	IN `precoCusto` decimal(10,2),
	IN `precoVenda` decimal(10,2),
	IN `dataInsercao` date,
	IN `obsProduto` varchar(255),
	IN `Categoria_codCategoria` int,
	IN `Marca_codMarca` int,
	IN `Fornecedor_codFornecedor` int
)
BEGIN
 	SELECT COUNT(*) INTO @contador FROM produto WHERE codProduto = codProdutos;
